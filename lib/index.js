...
export function apply(ctx) {
	...
	const diagnostics = { pings: {} };

	/** Record a client diagnostic ping (for debugging the client half). */
	function recordPing(name) {
		diagnostics.pings[name] = (diagnostics.pings[name] ?? 0) + 1;
	}
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			const ping = new URL(raw, "http://x").searchParams.get("ping");
			if (ping !== null) recordPing(ping);
